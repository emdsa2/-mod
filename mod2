...
(function () {
    ...
    mod.hookFunction("DrawCharacter", 10, (args, next) => {
        let data = args;
        let 角色名字 = args[0].Name
        let 房间人数 = ChatRoomCharacterCount
        let 房间内角色 = ChatRoomCharacter.map(character => character.Name);

        if (房间人数 === 2) {
            if (房间内角色[0] === 'tonic' && 角色名字 === 'tonic') {
                args[1] = 500
                ChatRoomCharacterDrawlist.sort((a, b) => {
                    if (a.Name === 'tonic') return 1;
                    if (b.Name === 'tonic') return -1;
                    return 0;
                });
    ...
    mod.hookFunction("ChatRoomDrawCharacterOverlay", 10, (args, next) => {
        DrawCharacter(ChatRoomCharacter[0], 500, 0, 1)
        next(args);
    });
