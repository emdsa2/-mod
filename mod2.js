...
(function () {
    ...
    function 动作拓展设置退出UIRun() {
        if (PreferenceSubscreen === "动作拓展设置" || PreferenceSubscreen === "自定义服装设置" || PreferenceSubscreen === "高潮计数保留设置") {
    ...
    function 动作拓展设置退出UIClick() {
        if (MouseIn(114, 75, 90, 90) && (PreferenceSubscreen === "动作拓展设置" || PreferenceSubscreen === "自定义动作设置" || PreferenceSubscreen === "自定义服装设置" || PreferenceSubscreen === "高潮计数保留设置")) {
            if (PreferenceSubscreen === "自定义动作设置") {
                笨蛋LZActivity();
    ...
    function 自定义动作设置Run() {
        if (PreferenceSubscreen === "自定义动作设置") { // 装备量表
            ...
            var name = document.getElementById('笨蛋Luzi_activityName')?.value || "";
            var targetSelfText = document.getElementById('笨蛋Luzi_targetSelfText')?.value || "";
            var targetText = document.getElementById('笨蛋Luzi_targetText')?.value || "";

            var targetSelf = Player.FocusGroup?.Name || "";
            var target = Player.FocusGroup?.Name || "";
            ...
            // DrawText(`条件`, 160, 593.33, "White");// 绘制一个文本元素

            // if (MouseIn(80, 545, 160, 100)) {
            //     DrawImageResize("https://emdsa2.github.io/-mod/image/白箭头右.png"
            //         , 220, 565, 90, 50);
            // }
            ...
            if (当前界面 == `删除`) {
                ...
                DrawBackNextButton(900, 325, 400, 64, 动作[当前动作索引], "White", "", () => { }, () => { });
        ...
        if (PreferenceSubscreen === "自定义动作设置") {
            ...
            if (当前界面 == `删除`) {
                ...
                if (MouseIn(1360, 325, 100, 64)) {
                    ...
                    var regex2 = new RegExp(动作[当前动作索引] + "$");
